...
def main():
    ...
    # タイトルの表示
    st.title("いづれあやめかかきつばた")

    # アップローダの作成
    uploaded_file = st.file_uploader(".jpgの画像を選択", type="jpg")
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  # あやめの確率(%)
        pred1 = round(prediction[1],3) * 100  # かきつばたの確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で{cls_name[0]}です！")
        elif classNo == 1:
            st.subheader(f"これは{pred0}％の確率で{cls_name[1]}です！")
...
# 学科：高度情報工学科
# 学年：4年
# コメント：いづれあやめかかきつばたのことわざを知り
# 人間にも見分けづらいものを見分けてもらうため
#=============================================================================#
